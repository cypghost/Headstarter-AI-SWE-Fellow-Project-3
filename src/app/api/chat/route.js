...
function replaceUrlsInText(text, urls, processed_data) {
  for (let i = 0; i < urls.length; i++) {
    text = text.replace(
      ...
      processed_data[i].id +
        " with " +
        processed_data[i].metadata["rating"] +
        " star rating in " +
        processed_data[i].metadata["department"]
    );
  }
  return text;
}
...
async function scrapeWebpage(url) {
  try {
    ...
    const lastName = $(
      "div.NameTitle__Name-dowf0z-0 span.NameTitle__LastNameWrapper-dowf0z-2"
    )
      .first()
      .text()
      .trim();
...
console.log(2); // Step 2: Define the system prompt
const systemPrompt = `
You are a rate your professor agent to help students find classes, that takes in user questions and answers them.
For every user question, the top 3 professors that match the user question are returned.
Use them to answer the question if needed.
`;

console.log(3); // Step 3: Create the POST function
export async function POST(req) {
  const data = await req.json();

  console.log(4); // Step 4: Initialize Pinecone and OpenAI
  const pc = new Pinecone({
    apiKey: process.env.PINECONE_API_KEY,
  });
  const index = pc.index("rate").namespace("ns1");
  ...
  const client = new AzureOpenAI({
    ...
    azure_endpoint: process.env.AZURE_OPENAI_COMPLETION_ENDPOINT,
  });
  const groq = new Groq({
    apiKey: process.env.GROQ_API_KEY,
  });

  console.log(5); // Step 5: Process the user’s query
  ...
  console.log(6); // Step 6: Query Pinecone
  ...
  console.log(7); // Step 7: Format the results
  let resultString = "";
  ...
  console.log(8); // Step 8: Prepare the OpenAI request
  const lastMessage = data[data.length - 1];
  const lastMessageContent = lastMessage.content + resultString;
  const lastDataWithoutLastMessage = data.slice(0, data.length - 1);

  console.log(9); // Step 9: Send request to OpenAI
  ...
  console.log(10); // Step 10: Set up streaming response
